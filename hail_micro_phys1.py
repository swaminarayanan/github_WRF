...
vert2 = ds2['qrain'].isel(time=16).mean(dim=['lat']).squeeze() / 1*1.0e+07
ept1 = ds3['qgraup'].isel(time=20).mean(dim=['lat']).squeeze() / 1*1.0e+07
ept2 = ds4['qgraup'].isel(time=16).mean(dim=['lat']).squeeze() / 1*1.0e+07
rel1 = ds5['qhail'].isel(time=20).mean(dim=['lat']).squeeze() / 1*1.0e+08
# xx = rel1.values 
rel2 = ds6['qhail'].isel(time=16).mean(dim=['lat']).squeeze() / 1*1.0e+08
div1 = ds7['qsnow'].isel(time=20).mean(dim=['lat']).squeeze() / 1*1.0e+07
# x = div1.values
div2 = ds8['qsnow'].isel(time=16).mean(dim=['lat']).squeeze() / 1*1.0e+07
...
gs = gridspec.GridSpec(2, 4, width_ratios=[1, 1, 1, 1], wspace=0.12)  # Adjusted to 3 columns
...
# Plot Temperature using contour (line contours with labels)
# div1_p = ax3.contour(div1['lon'], div1['lev'], div1, colors='k', linewidths=2, lats.min(), lats.max()
