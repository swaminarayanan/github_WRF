...
# Extract variables
vert1 = ds1['qcloud'].isel(time=20).mean(dim=['lat']).squeeze()/ 1*1.0e+07
vert2 = ds2['qcloud'].isel(time=16).mean(dim=['lat']).squeeze() / 1*1.0e+07
ept1 = ds1['qice'].isel(time=20).mean(dim=['lat']).squeeze() / 1*1.0e+07
ept2 = ds2['qice'].isel(time=16).mean(dim=['lat']).squeeze() / 1*1.0e+07
...
# fig, ax = p lt.subplots(nrows=1, ncols=2, figsize=(8, 10))
...
# Plot shaded CAPE using contourf (filled contour)
vert1_p = ax1.contourf(vert1['lon'], vert1['lev'], vert1, cmap='jet', 
                       levels=np.linspace(50, vert1.max(), 10))
...
# Plot shaded CAPE using contourf (filled contour)
rel1_p = ax3.contourf(ept1['lon'], ept1['lev'], ept1, cmap='YlOrBr', 
                                  levels=np.linspace(ept1.min(), ept1.max(), 10))
...
div1_p = ax3.contour(ept2['lon'], ept2['lev'], ept2, colors='k', linewidths=2, 
                      levels=np.linspace(ept2.min(), ept2.max(), 10))  # Proper LaTeX formatting
ax3.clabel(div1_p, inline=True, fontsize=15, fmt='%1.0f')
