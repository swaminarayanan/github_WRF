...
ds11 = ds['qcloud'].isel(time=list(range(16, 17)) + list(range(25, 26)) + list(range(32, 32)))
ds12 = ds['qice'].isel(time=list(range(16, 17)) + list(range(25, 26)) + list(range(32, 32)))  # Select the specific humidity variable
ds13 = ds['qgraup'].isel(time=list(range(16, 17)) + list(range(25, 26)) + list(range(32, 32)))  # Select the specific humidity variable
ds14 = ds['qhail'].isel(time=list(range(16, 17)) + list(range(25, 26)) + list(range(32, 32))) # Select the specific humidity variable
ds15 = ds['qrain'].isel(time=list(range(16, 17)) + list(range(25, 26)) + list(range(32, 32)))  # Select the specific humidity variable
ds16 = ds['qsnow'].isel(time=list(range(16, 17)) + list(range(25, 26)) + list(range(32, 32)))  # Select the specific humidity variable
#mogadishu equator
ds1 = ds11.mean(dim=['time', 'lat']) / 1*1.0e+07

# ds1 = ds1.values
ds2 = ds12.mean(dim=['time', 'lat']) / 1*1.0e+09
ds3 = ds13.mean(dim=['time', 'lat']) / 1*1.0e+07
ds4 = ds14.mean(dim=['time', 'lat']) / 1*1.0e+08
ds5 = ds15.mean(dim=['time', 'lat']) / 1*1.0e+07
ds6 = ds16.mean(dim=['time', 'lat']) / 1*1.0e+07
##################
s1 = ds1.values
s2 = ds2.values
s3 = ds3.values
...
levels1 = np.linspace(30, max_value1, 10)  # Adjust 100 for the number of levels you need
...
pcm1 = ax1.contourf(lons1, lev1, ds1, cmap='jet', levels = levels1)#, extend='both')#, levels=level)
...
x_ticks = np.arange(80, 91.1, 1)  # Create 10 evenly spaced ticks

x_ticks = x_ticks[:11]  # Limit to 12 ticks for the x-axis
# Format the x-axis labels to display months and years in 'MMM-YYYY' format
ax1.set_xticks(x_ticks)
ax1.set_xticklabels([f'{tick:.2f}' for tick in x_ticks], rotation=45)  # Format with 2 decimal places
...
# Create a contour plot with specified levels
pcm2 = ax2.contourf(lons2, lev2, ds2, cmap='jet', levels = levels2)#, extend='both')#, levels=level)
# Set x-axis ticks to display 12 monthly spaced ticks with year information
# x_ticks = np.linspace(start=lons2.min(), stop=lons2.max(), num=10)  # Create 10 evenly spaced ticks
x_ticks = np.arange(80, 90.1, 1)  # Create 10 evenly spaced ticks
x_ticks = x_ticks[:11]  # Limit to 12 ticks for the x-axis
# Format the x-axis labels to display months and years in 'MMM-YYYY' format
...
# Create a contour plot with specified levels
pcm3 = ax3.contourf(lons3, lev3, ds3, cmap='jet', levels = levels3)#, extend='both')#, levels=level)
...
# Create a contour plot with specified levels
pcm4 = ax4.contourf(lons4, lev4, ds4, cmap='jet', levels = levels4)#, extend='both')#, levels=level)
...
# Create a contour plot with specified levels
pcm5 = ax5.contourf(lons5, lev5, ds5, cmap='jet', levels = levels5)#, extend='both')#, levels=level)
...
ax5.set_xticklabels([f'{tick:.0f}' for tick in x_ticks], rotation=45)  # Format with 2 decimal places
...
# Create a contour plot with specified levels
pcm6 = ax6.contourf(lons6, lev6, ds6, cmap='jet', levels = levels6)#, extend='both')#, levels=level)
...
ax6.set_xticklabels([f'{tick:.0f}' for tick in x_ticks], rotation=45)  # Format with 2 decimal places
...
cbar1.ax.text(0.65, 1.05, '      × 10-⁷', fontsize=18, fontweight='bold', ha='center', transform=cbar1.ax.transAxes)
...
# Add the scale (× 10⁹) to the top of the colorbar
cbar2.ax.text(0.51, 1.05, '      × 10-⁹', fontsize=18, fontweight='bold', ha='center', transform=cbar2.ax.transAxes)
...
# Add the scale (× 10⁷) to the top of the colorbar
cbar3.ax.text(0.51, 1.05, '      × 10-⁷', fontsize=18, fontweight='bold', ha='center', transform=cbar3.ax.transAxes)
...
# Add the scale (× 10⁸) to the top of the colorbar
cbar4.ax.text(0.51, 1.05, '      × 10-⁸', fontsize=18, fontweight='bold', ha='center', transform=cbar4.ax.transAxes)
...
# Add the scale (× 10⁷) to the top of the colorbar
cbar5.ax.text(0.51, 1.05, '      × 10-⁷', fontsize=18, fontweight='bold', ha='center', transform=cbar5.ax.transAxes)
