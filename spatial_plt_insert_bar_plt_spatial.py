...
gs = gridspec.GridSpec(1, 1)#, width_ratios=[4, 4], hspace=0.061, wspace=0.0612)
...
inset_ax =  fig.add_axes([0.248, 0.61, 0.123, 0.19])  # [x, y, width, height] in relative figure coordinates
# bar = inset_ax.bar(categories, values, color=['blue', 'orange'], edgecolor='black')
inset_ax.bar(x, y, color=['r', 'k'])
inset_ax.set_title("hail size", fontsize=14)
# inset_ax.set_xlabel("data", fontsize=18)
inset_ax.set_ylabel("hail size (mm)", fontsize=14)
inset_ax.tick_params(axis='both', labelsize=8)
inset_ax.legend(fontsize=8)
# Customize tick labels for the inset plot
inset_ax.set_xticklabels(x, fontsize=13, fontweight='bold', rotation=0, ha='right')
inset_ax.set_yticklabels(inset_ax.get_yticks(), fontsize=13, fontweight='bold')# Add the circle at the specified position
# cir = Circle((lon, lat), radius, color='#FF6E00', fill=True, transform=ccrs.PlateCarree())
# ax2.add_patch(cir)
# Customize tick labels
################################
################################## note this ruffly aded script

inset_ax.set_xticks(range(len(x)))  # Ensure tick positions align with x
inset_ax.set_xticklabels(x, fontsize=10, fontweight='bold', rotation=0, ha='center')
inset_ax.set_yticks(range(0, max(y) + 2, 2))  # Set y-ticks dynamically
inset_ax.set_yticklabels(inset_ax.get_yticks(), fontsize=10, fontweight='bold')

# Optionally remove the legend or set its position
inset_ax.legend(["Observation", "Model"], fontsize=8, loc='upper right', frameon=False)
...
ax2.set_xticks(np.arange(82, 89.61, 1))
ax2.set_yticks(np.arange(21.5, 26.61, 1))
ax2.set_xlim([82, 88.6])
ax2.set_ylim([21.5, 26.61])
ax2.tick_params(axis='both', length=4, width=1.2)

# Modify the X-tick labels to show longitude with "E"
ax2.set_yticklabels([f'{tick:.0f}°N' for tick in np.arange(21.5, 26.61, 1)], fontsize=14, fontweight='bold', rotation=45)
ax2.set_xticklabels([f'{tick:.0f}°E' for tick in np.arange(82, 89.61, 1)], fontsize=14, fontweight='bold', rotation=45)
...
ax2.set_title('WRF model', fontsize=16, fontweight='bold')
...
cbar2 = plt.colorbar(pcm2, ax=[ax2], orientation='vertical', shrink=0.85)
cbar2.set_label('precipitation (mm)', fontsize=18, fontweight='bold')
...
cbar2.ax.set_position([0.75, 0.2625, 0.3, 0.55])  # Left, Bottom, Width, Height
